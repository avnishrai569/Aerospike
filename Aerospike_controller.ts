...
export class Aerospike_controller{
    ...
    constructor() {
       ...
       // In your controller class
        getAllClientsController = async (req: Request, res: Response): Promise<void> => {
            try {
                const allRecords = await this.aerospike_service.getAllClients();
                res.status(200).send(allRecords);
            } catch (error) {
                console.error("Error getting all records:", error);
                res.status(500).send("Error getting all records");
            }
        };
